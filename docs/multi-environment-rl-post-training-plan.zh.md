# Multi-Environment RL Post-Training 计划中文版

最后更新：2026-05-15

## 1. 背景与目标

这份计划基于当前 Nemotron 仓库中已经存在的 Super3、Nano3 和 Omni3 post-training recipe。仓库里已经具备一条可以复用的后训练主线：
...
- Omni3 路线：已有 multimodal RL 方向，包括 MPO、text GRPO 和 vision GRPO 数据准备路径；公开 recipe 只暴露了上游多模态环境的一部分。
...
- 性能目标不只看数学和代码，还要覆盖 agentic tool use、软件工程、chat/IF、长上下文、多语言，以及可选的多模态能力。
...
| 2026-10-17 到 2026-12-31 | **M3：年底匹配 Qwen/Qwen3.5-397B-A17B** | 环境扩到 70-100+，rollout 规模提升到百万级以上。加入 GUI/MCP/browser、deep SWE、代码安全、long-horizon workplace assistant、多语言 agent 和更难长上下文任务。如果多模态在 scope 内，接入 Omni-style MPO/vision/text RL，覆盖 OCR、document、chart、video、ASR 等环境。Agentic SFT v2 使用 M2 成功轨迹、负例修复、teacher reranking 和 GenRM reranking。最终 RL 分三波：高信号 RLVR、慢速 SWE/browser/GUI、最终 GenRM/RLHF。Infra 升级到 1K GPU-class async GRPO、env quota scheduler、sandbox pool、shadow eval 和自动回滚。 | 2026-12-31 前冻结最终 checkpoint。在约定 text/agent/coding basket 上匹配 Qwen3.5-397B-A17B，例如 BFCL、TAU2、VITA、DeepPlanning、Tool Decathlon、MCP-Mark、SWE-Bench、TerminalBench。若多模态是硬要求，还需纳入 MMMU/MMMU-Pro、MathVision、OCR、chart、document、video eval。 |
...
| M3 Expansion | 2026-10-17 到 2026-11-15 | GUI/browser/MCP/SWE/multimodal 环境扩展、Agentic SFT v2。 |
...
| Multimodal | MMMU/MMMU-Pro、MathVision、OCRBench、chart/document/video/ASR tasks |

**promotion gate。**

- M1：加权均分达到 Super3 parity，且关键类别无大回退。
- M2：选定 basket 上匹配 Qwen3.5-122B-A10B，关键单项 gap 控制在 3-5% 内。
- M3：年底 checkpoint 在 text/agent/coding basket 上匹配 Qwen3.5-397B-A17B；若多模态在 scope 内，必须通过 multimodal gate。
...
| Multimodal | OCR、document QA、chart、diagram、video、ASR、multimodal tool routing。 | 多模态答案正确性、OCR/ASR 精度、图表/视频 QA 分数。 |
...
| MPO | Mixed Preference Optimization，多模态偏好优化。Omni 路线中用于视觉/多模态 preference data。 |
| OCR | Optical Character Recognition，光学字符识别。用于图片、文档、截图中的文字理解。 |
| ASR | Automatic Speech Recognition，自动语音识别。用于音频转文字和语音理解。 |
| MMMU/MMMU-Pro | 多模态大学级理解 benchmark，常用于评估图文推理能力。 |
| MathVision | 多模态数学视觉 benchmark，考察图形、图片与数学推理结合能力。 |
| OCRBench | OCR 和文档/图片文字理解 benchmark。 |
