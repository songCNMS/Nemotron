# Task Knowledge

<!-- METADATA:SESSION=1 -->

- Scoped notebook:
  `usage-cookbook/Nemotron-3-Super/grpo-dapo/grpo_training_cookbook.ipynb`.
- Focused static test:
  `tests/usage_cookbook/test_super_grpo_dapo_rl_checkout_revision.py`.
- Required NeMo-RL docs revision:
  `bb0a7d43931950a74522e159f7117543a87b580b`.
- Mutable docs URLs to reject:
  `https://github.com/NVIDIA-NeMo/RL/blob/super-v3/docs/...` and
  `https://github.com/NVIDIA-NeMo/RL/blob/main/docs/...`.
- Preserve visible `super-v3` branch context prose; the executable checkout
  remains pinned and guarded by the previous task's contract.
