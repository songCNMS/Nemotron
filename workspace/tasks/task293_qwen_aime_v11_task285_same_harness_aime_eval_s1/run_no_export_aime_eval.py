#!/usr/bin/env python3
"""Task293 no-export/no-endpoint corrected AIME2025 eval for task285 iter2.

Run this on NemTron from a task-owned /root repo sync. The script loads the
task285 Megatron checkpoint directly, generates completions for the task247
corrected AIME2025 30x1 cache through the approved task291 in-process MCore
route, and scores with the same parser/normalizer/denominator as the task247
corrected endpoint runner.
"""
...
TASK_ID = "task293_qwen_aime_v11_task285_same_harness_aime_eval_s1"
ROUTE_ID = "direct_in_process_mcore_static_engine_no_export_no_endpoint_topk1_greedy_corrected_aime25"
BASE_SCORE = {"correct": 11, "total": 30, "accuracy": 0.36666666666666664}
DEFAULT_BASE_ARTIFACT_ROOT = Path(
    "/work-agents/intern_nemotron_worker_3/outputs/"
    "task247_qwen_aime2025_qwen4b_base_smoke_s1/"
...
def main() -> int:
    ...
    try:
        ...
        if os.environ.get("CUDA_VISIBLE_DEVICES") not in {"0", "0,"}:
            raise RuntimeError(
                "task293 requires exactly one visible GPU, got "
                f"CUDA_VISIBLE_DEVICES={os.environ.get('CUDA_VISIBLE_DEVICES')!r}"
            )
        if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
            raise RuntimeError(
                "task293 requires exactly one visible CUDA device, got "
        ...
        prompt_manifest = {
            ...
            "hf_chat_template_used": True,
            "hf_chat_template_enable_thinking_false": True,
            "prompt_token_match_against_task247_base": prompt_token_matches,
        ...
        protocol_proof = {
            ...
            "same_parser_normalizer": (
                ...
                "from task071 run_corrected_math_full_eval.py used by task247"
            ),
            "same_all_request_denominator": True,
            "same_max_tokens": args.max_tokens == 8192,
            "prompt_token_mismatch_count": prompt_manifest["prompt_token_mismatch_count"],
            "prompt_tokens_match_task247_base": prompt_manifest[
                "prompt_token_mismatch_count"
            ]
            == 0,
            "generation_backend": "MCore in-process static engine; no endpoint by task293 boundary",
            "base_generation_backend": "SGLang /v1/chat/completions endpoint from task247 artifact",
            "sampling_exact_parameter_match": False,
            "sampling_semantic_match": (
                "deterministic greedy argmax: task247 used temperature=0/top_p=1e-5; "
                "task293 local MCore used top_k=1 greedy branch with temperature=1/top_p=0"
            ),
        }
        ...
        summary = {
            ...
            "route": ROUTE_ID,
            "source_head": args.source_head,
            "model": "task285-qwen3-4b-v11-ft-iter0000002",
            "checkpoint_iter_dir": str(args.checkpoint_iter_dir),
            "base_model_path": str(args.base_model_path),
        ...
        print(f"TASK293_DISPOSITION={disposition}")
        print(f"TASK293_FT_SCORE={ft_correct}/{ft_total}")
        print(f"TASK293_OUTPUT_ROOT={output_root}")
        return 0
        ...
        blocker_path = eval_dir / "aime_blocker.json"
        ...
        print("TASK293_DISPOSITION=BLOCK")
        print(f"TASK293_BLOCKER={type(exc).__name__}: {exc}")
        print(f"TASK293_OUTPUT_ROOT={output_root}")
        return 2
