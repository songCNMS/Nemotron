...
5. task328 preflight result: the prior constrained task299 packed root remains
   the only safe carry-forward packed root; it has Qwen3-30B contract,
   split-parity, decontam, and no-AIME train leakage proof.
6. task322 `instruction-following-structured`, task322 `agentic-interactive`,
   and task327 `swe` are raw decontam-pass sources but are excluded from the
   task328 packed contract because split exposure/parity and Qwen3-30B
