...
10. If an approved downstream PR head advances before merge, request a new worker mailbox report for that head; if an authorized external merge occurs before the report arrives, record the final head, merge commit, mergedBy, and the remaining mailbox-evidence gap explicitly.
