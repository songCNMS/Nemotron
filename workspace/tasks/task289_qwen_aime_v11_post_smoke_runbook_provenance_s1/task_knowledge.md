...
10. task293 same-harness proof: accepted task247 AIME cache, `0` prompt-token
...
11. task293 residual risk: `sampling_exact_parameter_match=false` because the FT
    eval used the no-export/no-endpoint MCore greedy route while the accepted
    base comparator used SGLang `/v1/chat/completions`.
12. No task294 PR or branch is visible from `gh pr list --state all --search
    task294` and `git ls-remote --heads origin '*task294*'`; record task294
    independent review as pending/not visible until lead or repo state shows it.
