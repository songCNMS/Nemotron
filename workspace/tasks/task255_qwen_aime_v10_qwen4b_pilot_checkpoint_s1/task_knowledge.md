...
9. task256 could not review those `/root` artifact paths directly, so task258
   must provide reviewer-readable artifact access or a precise resource/access
   blocker without changing model contents.
10. task258 reviewer-readable bundle for the task255 artifacts is
    ...
    It contains the full checkpoint and HF export plus `review_manifest/`.
