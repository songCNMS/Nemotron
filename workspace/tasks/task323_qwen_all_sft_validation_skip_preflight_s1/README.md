...
- Recommendation: `PASS_ROUTE_A_PREFLIGHT`.
- Result: task-owned dereferenced train-only root exposes `46` train parquets,
  `0` valid parquets, `0` test parquets, `0` symlinks, and records
  `do_validation=false` plus required same-harness eval handoff.
- Boundaries: no training, optimizer steps, benchmark eval, export, endpoint,
  promotion, final packing, product-code edit, task255 reuse, AIME2025 train
