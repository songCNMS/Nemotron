...
|---|---|---|---|
| 1 | `prepare_m1_rlvr_jsonl.py` (M0 → RLVR1 JSONL bridge，NeMo-Gym env map，lineage 接 M0 manifest) | yes | ✓ Done (this PR) |
| 2 | RLVR1 config wiring + smoke launcher (`nemotron super3 rl rlvr1 -c smoke`) — flip `data_prep/rlvr1.yaml` 指向 M0-derived artifact | partial (config syntax 可以验证 sandbox，真 launch 走集群) | Todo |
