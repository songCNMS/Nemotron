...
runtime/resource route. The runtime/resource history is present from prior
merged work, but the fresh task308/task309 PASS artifacts are not visible or
accepted, so task310 fails closed before any training command.
...
|---|---|---|---|
| task308 all-SFT pipeline inventory audit | `PASS_AUDIT` with inventory report and accepted evidence | Branch `intern_nemotron_worker_1/task308_qwen_all_sft_pipeline_inventory_audit_s1` exists at `348cba44c02043cd6310a36ec722a68278288db2`; only task README/history/knowledge/status files are visible; no PR and no `all_sft_pipeline_inventory_audit_report.md` found | Blocks training |
| task309 all-SFT packed-data contract | `PASS_PACKED_CONTRACT` with packed root, manifests, decontam proof, checksums, and accepted evidence | Branch `intern_nemotron_worker_2/task309_qwen_all_sft_packed_data_contract_s1` exists at `d054925b1792a5365738247eeb8bdec462e1e6c6`; only task README/history/knowledge/status files are visible; no PR and no `all_sft_packed_data_contract_report.md` found | Blocks training |
...
acceptance for the packed-data contract. After those gates pass, task310 should
refresh the 30B runtime/resource assumptions against the current main and exact
packed root before launching.
