...
| 7 | no difficulty curriculum / pass-rate filtering | P2 | ✓ Fixed in PR #14 task008 — `load_difficulty_signal` reads M0 `health_baseline_report.json`; each SFT row carries `metadata.difficulty_bucket` ∈ {trivial, hard, unknown}; `manifest.difficulty_buckets` summarizes counts per split. v0 still emits a single blend entry — a future curriculum sampler can stratify on the new metadata. |
| 8 | chat template pinned to `nano3` | P3 | ✓ Fixed in task012 — `src/nemotron/data_prep/templates/super3.jinja` ships as a verbatim copy of nano3; resolver `_apply_chat_template` learned the `super3` name; `stage1_sft/config/data_prep/{default,agentic_v0,tiny}.yaml` now declare `chat_template: super3`. Render-time tests cover the four-role conversation + tool-repair negative round-trip. Diverge `super3.jinja` from `nano3.jinja` as Super3-specific behavior is identified. |
