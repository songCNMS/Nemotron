...
def transform_musique_search(row: Mapping[str, Any], spec: Mapping[str, Any]) -> JsonDict:
    ...
    layout is reused. The two extra MuSiQue-only fields (decomposition,
    is_supporting flag, answer aliases) land in `extra_env_info` so downstream
    consumers (M1 RLVR, eval) can use them.
    """
